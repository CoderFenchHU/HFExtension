...
static char * const  kOriginSize = "kOriginSize";

@interface UIFont (privateAdjustSize)
@property (nonatomic, assign) CGFloat originSize;
@end

@implementation UIFont (privateAdjustSize)

- (CGFloat)originSize {
    return [objc_getAssociatedObject(self, kOriginSize) floatValue];
}

- (void)setOriginSize:(CGFloat)originSize{
    objc_setAssociatedObject(self, kOriginSize, @(originSize), OBJC_ASSOCIATION_ASSIGN);
}
...
- (UIFont *)originFont{
    NSLog(@"selfOriginSize: %f", self.originSize);
    return [UIFont swizz_fontWithName:self.fontName size:self.originSize];
}
